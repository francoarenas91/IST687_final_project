...
PAQUETES <- c("dplyr","tidyverse","ggplot2","here","arrow","lubridate","readxl","randomForest","doParallel","caret","plm")
...
feature exploration
```{r}
encoded_house_data %>% select(in.clothes_dryer) %>% unique()

# transform the income strings into numbers
encoded_house_data %>% select(in.income) %>%
  unique() %>% 
  mutate(
    income_lower_bound=str_extract(in.income, "^[^-]*"),
    income_lower_bound=str_replace(income_lower_bound,"<10000","5000"),
    income_lower_bound=str_replace(income_lower_bound,"200000\\+","200000"),
    income_lower_bound=as.numeric(income_lower_bound)
  ) %>% 
  arrange(income_lower_bound) -> income_dictionary

encoded_house_data %>% select(in.usage_level) %>%
  unique()

encoded_house_data %>% select(in.plug_load_diversity) %>%
  unique()

encoded_house_data %>% select(in.ducts) %>%
  unique()

encoded_house_data %>% select(in.insulation_wall) %>%
  unique()

encoded_house_data %>% select(in.windows) %>%
  unique()

encoded_house_data %>% select(in.hvac_cooling_efficiency) %>%
  unique()

encoded_house_data %>% select(in.geometry_garage) %>%
  unique()

encoded_house_data %>% select(in.city) %>%
  unique()

encoded_house_data %>% select(in.window_areas) %>%
  unique()
encoded_house_data %>% select(in.orientation) %>%
  unique()

encoded_house_data %>% select(in.insulation_ceiling) %>%
  unique()
encoded_house_data %>% select(in.pv_system_size) %>%
  unique()
encoded_house_data %>% select(in.cooling_setpoint_offset_period) %>%
  unique()

encoded_house_data %>% select(in.geometry_wall_exterior_finish) %>%
  unique()
encoded_house_data %>% select(in.ceiling_fan) %>%
  unique()
...
data <-read_csv("C:/Users/franc/OneDrive/MADS/IST687/final_project/electricity_consumption.csv") %>%
  ...
  mutate(bldg_id=factor(bldg_id))

data %>%
  select(-county_id,in.county) %>% #remove the  the county in code
  mutate(day = day(time)) %>%
...
We can get rid of some features
```{r}
selected_importance_names_2<-readRDS("~/GitHub/household_energy_consumption/selected_importance_names_2.RDS")

data %>% 
  select(any_of(selected_importance_names_2),total_electricity,bldg_id) %>% 
  select(-in.weather_file_longitude,-in.weather_file_latitude) -> data #colinear with other features
...
try a linear regression. There is not test train split, because as we are triyng to predict each individual consumption for each hour, overfitting is not an issue (is desirable)
```{r}
# # Set up cross-validation
# train_control <- trainControl(method="cv", number=10)

# Train the model with cross-validation
lm_model <- lm(total_electricity ~ ., data=data)


summary(lm_model)

saveRDS(lm_model,"~/GitHub/household_energy_consumption/models/lm_model.RDS")

# Predict on the test set
predictions <- predict(lm_model, testset)

# Compute RMSE for the test set
rmse_test <- sqrt(mean((testset$total_electricity - predictions)^2))

# Output the RMSE
print(rmse_test)
```

this is esentially a panel poblem with fixed effect. In the panel it makes not sense a train-test split, because what we are trying to predict is the energy consumtion for specific dates for each household, and we do not have data of several years for the same dates.
```{r}
panel_data <- pdata.frame(data, index = c("bldg_id", "time"))
rm(data)

#run a fixed effects model
panel_model <- plm(total_electricity ~ ., 
                data = panel_data, 
                model = "within")
